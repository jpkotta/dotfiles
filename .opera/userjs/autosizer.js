// ==UserScript==
// @name Autosizer
// @author Arve Bersvendsen, Edited version by shoust, should detect all images and resize.
...
// shoust's version: http://my.opera.com/shoust/blog/2007/06/28/my-userjs-modified-userjs
// ==/UserScript==
 ...
 */
 ...
 //Updated to work for opera 9.5, now added small fix for 10.5
(function(){if( document.body && document.getElementsByTagName('link')[0] && (document.getElementsByTagName('link')[0].getAttribute('href').match(/style[s]?\/image\.css$/) || !document.getElementsByTagName('head')[0]) ) {
  var allBodyParts = document.body.getElementsByTagName('*');
  var firstBodyChild = document.body.firstChild;
  if( ((allBodyParts.length==5 || allBodyParts.length==2) && firstBodyChild.tagName.match(/^(table|div)$/i) && (allBodyParts[1].tagName == 'IMG' || allBodyParts[4].tagName == 'IMG'))) {
   window.addEventListener('load', function(ev){
    ...
    var fit_modes = [
      ...
      "display:block;max-width:100%;max-height:100%;margin:0 auto;cursor:crosshair", //fit to window
      "display:block;width:100%;height:auto;margin:auto;padding:0;cursor:crosshair", //fit to width
    ...
    document.documentElement.style="text-align:center;vertical-align:middle;padding: 0;margin: 0 auto;height:100%;width:100%;position:relative;min-height:100%;";
    ...
    for (var i = itemNames.length-1; i >-1;i--) {
      ...
      item.style="display:block;margin:0;padding:0.5em;text-align:left";
